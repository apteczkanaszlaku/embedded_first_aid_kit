...
void nfc_task(void *pvParameter)
{
    ...
    int versiondata = pn532_getFirmwareVersion(&nfc);
    if (!versiondata)
    {
	...
	uint8_t rx_buffer[255]; // receive nfc
	uint8_t tx_buffer[255]; // transmit nfc
    ...
    while (1)
    {
        ...
        /*
         uint8_t rx_len = 0;
         // receive initiator nfc data
        if (pn532_getDataTarget(&nfc, rx_buffer, &rx_len)) {
            ESP_LOGI(TAG, "Data received: (length: %d):", rx_len);
            for (int i = 0; i < rx_len; i++) {
                ESP_LOGI(TAG, "0x%02X ", rx_buffer[i]);
            }

            const char *response = "200"; 
            memcpy(tx_buffer, response, strlen(response));
            pn532_setDataTarget(&nfc, tx_buffer, strlen(response));

            ESP_LOGI(TAG, "Data sent %s", response);
        } else {
            ESP_LOGI(TAG, "Waiting for intiator data...");
        }
        */
    }
}

void pn532TaskStart(void){
	    xTaskCreatePinnedToCore(&nfc_task,"nfc_task", NFC_STACK_SIZE, NULL, NFC_PRIORITY, NULL, NFC_CORE_ID);
}